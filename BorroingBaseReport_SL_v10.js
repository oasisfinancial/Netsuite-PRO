	...
	try {
		if ( request.getMethod() == 'GET' ) {
			...
			if ( customers && customers.length > 0 ) {
				filters = [ [ 'category', 'is', '2' ], 'and', [ "internalid", "anyof", customers ] ];
			} else {
				filters = [ [ 'category', 'is', '2' ] ]
			}
			...
			do {
				...
				for ( var j = 0; j < tmp.length; j++ ) {
					...
					if ( billstate && due_to_heir ) {
						var estateobj = { "billstate": billstate, "duetoheir": due_to_heir };
						estates[ id ] = estateobj;
					} if ( billstate ) {
						var estateobj = { "billstate": billstate, "duetoheir": '' };
						estates[ id ] = estateobj;
					} else if ( due_to_heir ) {
						var estateobj = { "billstate": '', "duetoheir": due_to_heir };
						estates[ id ] = estateobj;
					} else {
						var estateobj = { "billstate": '', "duetoheir": '' };
						estates[ id ] = estateobj;
					}
				}
				resultIndex = resultIndex + resultStep;
			...
			var properties = [], assets = [], claims = [], c_leins = [], e_leins = [],c_existing_assignments = [],c_existing_assignments1 = [], e_existing_assignments = [];
			var propertiesfilter = [];
			var assign_properties={};
			if ( customers && customers.length > 0 ) {
				propertiesfilter = [ [ "custrecord_existing_assignment_customer", "anyof", customers ] ]
			}
			...
			var estatefilter = [];
			if ( customers && customers.length > 0 ) {
				estatefilter = [ [ "custrecord_property_estate", "anyof", customers ] ];
			}
			...
			var assignestate = [];
			if ( customers && customers.length > 0 ) {
				assignestate = [ [ "custrecord_property_estate", "anyof", customers ] ];
			}
			...
			var assetfilter = [];
			if ( customers && customers.length > 0 ) {
				assetfilter = [ [ "custrecord_asset_estate", "anyof", customers ] ];
			}
			...
			var claimfilter = [];
			if ( customers && customers.length > 0 ) {
				claimfilter = [ [ "custrecord_claim_estate", "anyof", customers ] ];
			}
			...
			var leinfilter = [];
			if ( customers && customers.length > 0 ) {
				leinfilter = [ [ "custrecord_lein_judgement_customer", "anyof", customers ] ];
			}
